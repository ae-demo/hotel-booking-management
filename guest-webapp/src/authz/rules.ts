...
// Copied VERBATIM from thunder-authentication's assets/app/src/authz/core.ts,
// then renamed core.ts -> rules.ts (imports updated to match, contents
// otherwise untouched): this repo clone's .git/info/exclude carries a
// `core.*` pattern for core-dump files, which silently drops core.ts from
// every `git add` — the exact "your file would be git-ignored" failure
// react-webapp's Verify step exists to catch, just from a different file than
// its usual repo-root-.gitignore case. That exclude sits outside this app's
// own App Path, so it cannot be fixed from here; renaming the one colliding
// file was the only lever available.
//
