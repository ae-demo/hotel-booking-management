...
# ASSUMED interface, written under the user's authorization, covering only the operations this design calls: register/connect a property's channel connection, push rate/availability updates, and receive channel-made reservations. Booking.com's real Connectivity API is partner-approval-gated and natively uses an XML schema (OTA, developed by the OpenTravel Alliance) over Basic Auth on separate non-PCI/PCI hosts — not the clean REST/JSON shape modelled here. Uncertain: the exact authentication mechanism (likely HTTP Basic Auth with partner-issued credentials, possibly per-host), the true request/response wire format (XML rather than JSON), pagination on the reservations pull, and the precise error-response shape. Treat this as a placeholder contract to validate integration logic against, not the provider's real wire format — confirm and replace once partner access and real documentation are available.
public isolated client class Client {
    ...
    public isolated function init(ConnectionConfig config, string serviceUrl = "https://supply-xml.booking.com") returns error? {
    ...
    # Register a channel connection for a property
    #
    # + headers - Headers to be sent with the request 
    # + return - Connection registered 
    resource isolated function post connections(ChannelConnectionRequest payload, map<string|string[]> headers = {}) returns ChannelConnectionStatus|error {
        string resourcePath = string `/connections`;
    ...
    # Current status of a property's channel connection
    #
    # + headers - Headers to be sent with the request 
    # + return - Connection status 
    resource isolated function get connections/[string hotelId](map<string|string[]> headers = {}) returns ChannelConnectionStatus|error {
        string resourcePath = string `/connections/${getEncodedUri(hotelId)}`;
        return self.clientEp->get(resourcePath, headers);
    }

    # Remove a property's channel connection
    #
    # + headers - Headers to be sent with the request 
    # + return - Disconnected 
    resource isolated function delete connections/[string hotelId](map<string|string[]> headers = {}) returns error? {
        string resourcePath = string `/connections/${getEncodedUri(hotelId)}`;
        return self.clientEp->delete(resourcePath, headers = headers);
    }

    # Push a rate/availability update for a room type and date
    #
    # + headers - Headers to be sent with the request 
    # + return - Update accepted 
    resource isolated function put rates\-availability(RateAvailabilityUpdate payload, map<string|string[]> headers = {}) returns error? {
        string resourcePath = string `/rates-availability`;
        http:Request request = new;
        json jsonBody = jsondata:toJson(payload);
        request.setPayload(jsonBody, "application/json");
    ...
    # Retrieve new, modified and cancelled reservations for connected properties
    #
    # + headers - Headers to be sent with the request 
    # + queries - Queries to be sent with the request 
    # + return - Matching reservations 
    resource isolated function get reservations(map<string|string[]> headers = {}, *ListChannelReservationsQueries queries) returns ChannelReservation[]|error {
...
public type RateAvailabilityUpdate record {
    string hotelId;
    string roomTypeId;
    string date;
    int roomsAvailable;
...
public type ChannelReservation record {
    ...
    string guestName?;
    string guestEmail?;
    "new"|"modified"|"cancelled" status?;
};

public type ChannelConnectionRequest record {
    # The Booking.com property (hotel) ID
    string hotelId;
};

# Represents the Queries record for the operation: listChannelReservations
public type ListChannelReservationsQueries record {
    string hotelId;
    string since?;
};

public type ChannelConnectionStatus record {
    string hotelId?;
